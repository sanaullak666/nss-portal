  ...
  if (nodemailer && (process.env.SMTP_HOST || process.env.GMAIL_USER)) {
    try {
      ...
      await transporter.sendMail({
        from: `"PU NSS Portal" <${process.env.GMAIL_USER || process.env.SMTP_USER || 'no-reply@pondiuni.edu.in'}>`,
        to: toEmail,
      ...
      console.error('[EMAIL SEND ERROR] Could not send via nodemailer:', err.message);
      return true;
    }
  }
